...
res_net_server = resnet18_server(cutlayer).to(device)
res_net_client = resnet18_client(cutlayer).to(device)
...
lr = 0.001
optimizer = optim.SGD(res_net_server.parameters(), lr=lr, momentum=0.9)
clientsoclist = []
train_total_batch = []
val_acc = []
client_weights = copy.deepcopy(res_net_client.state_dict())
...
for r in range(rounds):
    for user in range(users):
        # global_weights_cpu = {k: v.cpu() for k, v in client_weights.state_dict().items()}
        ...
        for local_epoch in range(local_epochs):
            ...
            train_loss_round = []
            train_acc_round = []
            for i in tqdm(range(train_total_batch[user]), ncols=100, desc='Round {} Epoch {} Client{} '.format(r+1, local_epoch, user)):  
                ...
                output = res_net_server(client_output)  # forward propagation
                ...
                optimizer.step()
            train_loss_temp = train_loss / train_total_batch[user]
        ...
        with torch.no_grad():
            ...
            for j, val in enumerate(testloader):
                ...
                server_model_dict = copy.deepcopy(res_net_server.state_dict())
                whole_model_temp = copy.deepcopy(client_model_dict)
                whole_model_temp.update(server_model_dict)  #
                res_net_whole.load_state_dict(whole_model_temp)
            ...
            df.to_csv(f'./splitlearning_{args.lr}_{args.bs}_{args.local_ep}_{args.num_users}_{args.choosen_number}_{args.niid}_{args.rounds}.csv')
...
PATH = './cifar10_sp_resnet18_server.pth'
torch.save(res_net_server.state_dict(), PATH)
...
df.to_csv(f'./slcommunication_{args.lr}_{args.bs}_{args.local_ep}_{args.num_users}_{args.choosen_number}_{args.niid}_{args.rounds}.csv')  
