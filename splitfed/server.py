        ...
        with lock: 
            if weight_count == num_users:
                for i, conn in enumerate(clientsoclist):
                    choosen_cut_layer = args.cutlayer
                    cutlayers[i] = choosen_cut_layer
                    res_net_whole.load_state_dict(whole_model_weights)
                    res_net_client_new = resnet18_client(choosen_cut_layer).to(device)
        ...
        with lock:
            ...
            if weight_count == num_users:
                ...
                with torch.no_grad():
                    ...
                    df.to_csv(f'./splitfed_{args.lr}_{args.bs}_{args.local_ep}_{args.num_users}_{args.choosen_number}_{args.cutlayer}_{args.niid}_{args.rounds}.csv')
...
print(res_net_whole,res_net_client,res_net_server)
...
testset = torchvision.datasets.CIFAR10 (root=root_path, train=False, download=True, transform=transform_train)
...
res_net_client_new = resnet18_client(2).to(device)
